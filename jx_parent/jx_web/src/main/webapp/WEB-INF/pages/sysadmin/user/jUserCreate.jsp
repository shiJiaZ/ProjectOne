...
function change(){
	//获取选中部门的id
	//var deptId = $("#deptId>option").val();
	var deptId = $("#deptId :selected").val();
	//alert(deptId);
	
	/* $.ajax({
		url:'userAction_findManagers?deptId='+deptId,
		dataType:'text',
		type:'get',
		success:function(data){
			//清空managers下拉列表框
			//使用jQuery获取对象都是数组对象，立面存放的是一个个js对象，可以通过下标的方式访问这些对象，取出来的就是js对象
			$("#managers")[0].length=1;
			//alert(data);
			//分割data,使之成为数组
			data = eval("("+data+")");
		...
		<table class="commonTable" cellspacing="1">
       		<tr>
	            ...
	            <td class="tableContent">
	            	<s:select name="dept.id" list="deptList" id="deptId" onchange="change()"
        	...
        	<tr>
	            ...
	            <td class="tableContent">
	                <!-- c：jstl   java server tag librarys 
	                     c/fmt/fn/el 标签、s标签、shiro标签等都属于服务器标签，是需要tomcat服务器去解析的，服务器将他们解析成静态资源之后，返回给客户端访问
	            	...
	            	<%-- <s:select name="userInfo.manager.id" list="userList"
	            		...
	            		headerKey="" headerValue="--请选择--"
	            	></s:select> --%>
	            	<select name="userInfo.manager.id" id="managers">
